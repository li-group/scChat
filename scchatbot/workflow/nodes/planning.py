...
class PlannerNode(BaseWorkflowNode):
    ...
    def _build_planning_prompt(self, message: str, available_cell_types: List[str], 
        ...
        return f"""
                ...
                Create a step-by-step execution plan (JSON) for the user query.
                
                CONTEXT:
                - Available cell types ({len(available_cell_types)}): {', '.join(sorted(available_cell_types))}
                - Failed discoveries: {', '.join(sorted(unavailable_cell_types)) if unavailable_cell_types else "None"}
                - Status: {'Expanded' if len(available_cell_types) > len(self.initial_cell_types) else 'Initial only'}
                ...
                DECISION LOGIC:
                1. GENE/MARKER (differences, markers, counts) → DEA (dea_split_by_condition, compare_cell_counts)
                2. PATHWAY (functional, ontology, signatures) → Enrichment (perform_enrichment_analyses)
                3. CELL ABUNDANCE (proportions, quantities) → Counting (process_cells, compare_cell_counts, stacked plots)
                4. VISUALIZATION ONLY (e.g. "show plot") → Set "visualization_only": true.
                5. CONVERSATIONAL → No analysis steps.

                JSON FORMAT:
                {{
                    "plan_summary": "Brief strategy description",
                    ...
                    "steps": [
                        {{
                            ...
                            "target_cell_type": "Cell type name"
                        }}
                    ]
                }}
                
                CRITICAL RULES:
                1. CELL TYPES: Only plan for "Available cell types". If missing, discover first (process_cells). Skip unavailable types.
                2. GRANULARITY: Analyze one cell type per step (e.g., Step 1: T cell, Step 2: B cell). Never use list "T cells, B cells".
                3. ORDER: "dea_split_by_condition" MUST precede "display_dea_heatmap" for the same cell type.
                
                TOOL INSTRUCTIONS:
                - Enrichment (perform_enrichment_analyses):
                  * MINIMAL params: {{"cell_type": "Name"}}. NO "analyses" or "pathway_include" (auto-detected).
                
                - Visualizations:
                  * Enrichment: "display_enrichment_visualization" (analysis="go"|"gsea", plot_type="bar"|"dot"|"both").
                  * Cell Counts (Aggregate): "display_cell_count_comparison" (after multiple compare_cell_counts).
                  * Cell Counts (Stacked): "display_cell_count_stacked_plot" (for clinical/treatment groups, list all cell_types).
                  * DEA Heatmap: "display_dea_heatmap" (IMMEDIATELY after DEA step).
                  * UMAP (Gene): "display_feature_plot" (genes=["A","B"]).
                  * UMAP (Cluster): "display_leiden_umap" (shows cluster structure).
                  * UMAP (Global): "display_overall_umap" (color_mode="cell_type"|"accumulative_leiden").
                  * Expression (Dist): "display_violin_plot" (compare conditions, params: cell_type, genes).

                - Semantic Search (search_enrichment_semantic):
                  * Use for specific pathway queries not in top results or context-based search.
                  * Params: query="term", cell_type="Name".
                """
